...
* [◀️ Diario di ieri](./2025-11-25_IT.md)
...
Il pannello ieri sera è stato aggiustato inserendo i campi
...
Si può fare differenza se il dato da estrarre è uno solo
ed è indicizzabile, si può mettere come ultimo dato nell'indice
...
e vanno via millisecondi che è na bellezza)
...
un tinyint(1) che può essere 0==false altrimenti true.
Il campo in mappa è di "azione", non viene memorizzato
in un archivio da cui può essere recuperato ma agisce
a comandare tutti i record della tabella contest_votes
che hanno lo stesso section_id work_id.
Nel componente andrà gestito un campo a uso interno.

## URGENT REFACTOR

È la separazione in /contest/build e contest/manage
...
Si può anche avere un manage/jury e un manage/organization
oppure tutto quello che riguarda la giuria deve andare sotto /jury

In effetti avevo anche già messo un avviso:
/contest è per la build del concorso
/organization/contest è dedicato alla gestione del concorso da parte dell'organizzazione
...
```sh
✅ php artisan livewire:move --help
Description:
...
🛑  php artisan livewire:move Contest/Jury/Board.php Jury/SectionBoard.php
🛑  php artisan livewire:move Livewire/Contest/Jury/Board.php LiveWire/Jury/SectionBoard.php
🛑  php artisan livewire:move /app/Livewire/Contest/Jury/Board.php app/LiveWire/Jury/SectionBoard.php
🛑  php artisan livewire:move /app/Livewire/Contest/Jury/Board app/LiveWire/Jury/SectionBoard
✅  php artisan livewire:move Contest/Jury/Board Jury/SectionBoard
 COMPONENT MOVED  🤙
...
‼️ Fatto il trasferimento web.php diventa *automaticamente* errato perché il vecchio percorso
è stato sostituito, serve quindi sostituirlo subito altrimenti i successivi comandi
vengono interrotti per errore. Se il percorso manca in web.php nessun problema,
vanno però modificate le chiamate @livewire nei file blade.

Alt! non Jury collettivo ma juror singolare il voto è del giurato
...
qui c'è da riprender eil concetto. Possono accedere ai pannelli
...
JurorOnlyPolicy intendendo che il pannello è riservato ai giurati.

Ma dove vado a prendere il giurato è una domanda lecita, in quanto lo
user_roles.role comprende 'juror' ed è autoassegnabile mentre 
contest_jury.user_contact_id viene assegnato dall'organizzazione
e non è autoassegnabile, sembra quindi scontato che passi per la verifica in
...
Aggiornato AppServiceProvider, il controllo si può mettere sia nei controller
che nella blade per impedire l'acceso ai controller.
Vanno messi entrambi perché il link si può copincollare.

Alla stessa maniera posso inserire un gate per le pagine cui deve
accedere qualcuno dell'organizzazione, in questo caso sì,
