...
[questo](./2025-12-26_dbml.txt). DBML come html è un sistema di definizione
anzi marcatura dedicato agli archivi.

La rivoluzione di user_contacts.user_id diventa user_contacts.id
quando la faccio? Quando saprò TUTTI i punti in cui intervenire.
...
[Excel Partecipanti ed Esiti](./2025-12-26_ESEMPIO_FOTO_Partecipanti_ed_ESITI_2019_2019Y1.xls)  
...
* H | Autore Città  
Località da indicare nella spedizione postale
...
E per raccogliere i dati della FIAF devo prima definire che dati (2) vanno raccolti
...
Trovo che esiste già il package per la verifica del codice fiscale, scritto
da [Roberto Gallea](https://github.com/robertogallea/laravel-codicefiscale)  
...
Quando deciderò di usarlo dovrò aggiungere ai campi FIAF quelli utili per la verifica del codice fiscale:

* first_name (c'è)
* last_name (c'è)
* birthdate (manca)
* place (of birthdate) (manca)
* gender (M|F) (manca)
...
### Creare tutto in un colpo solo

```sh
$ php artisan make:model FederationAdditional --all

  ✅ INFO  Model [app/Models/FederationAdditional.php] created successfully.  

  ✅ INFO  Factory [database/factories/FederationAdditionalFactory.php] created successfully.  

  ✅ INFO  Migration [database/migrations/2025_12_26_185529_create_federation_additionals_table.php] created successfully.  

  ✅ INFO  Seeder [database/seeders/FederationAdditionalSeeder.php] created successfully.  

  ❓ INFO  Request [app/Http/Requests/StoreFederationAdditionalRequest.php] created successfully.  

  ❓ INFO  Request [app/Http/Requests/UpdateFederationAdditionalRequest.php] created successfully.  

  ❓ INFO  Controller [app/Http/Controllers/FederationAdditionalController.php] created successfully.  

  ❓ INFO  Policy [app/Policies/FederationAdditionalPolicy.php] created successfully.  
```

I primi ok grazie, ma gli altri? che uso possono avere? Per ora li elimino subito.

OK, fatta anche la tabella dei campi KV, perfino.  
La richiesta dei dati da inserire in User_contacts deve diventare una serie di pannelli, così
chiedo prima dei dati, e se dai dati di associato a federazione risulta che uno è italiano
oppure tesserato FIAF, allora scatta ala richiesta dei campi in più, richiesti da FIAF.

Mettere in testa: nazione (ok, c'è già) e *appartenenza a una federazione*.

Per l'appartenenza a una federazione si può prevedere anche il contestuale
check se in user_roles c'è l'id di user_contacts + 'member' + l'id della federazione.
