...
1. User  
  ...
  Tabella utenti con password. Qui non vengono memorizzati
  altri dati, solo quelli che consentono l'accesso alla
  piattaforma, gli altri dati, perché non vengano esposti
  questi, sono in una tabella Contact.
...
1. Contest  
  ...
  i dati possono essere inseriti in un primo record circuito
...
1. ContestSection  
  ...
  anche in questo caso formato uuid, con contest id.
1. ContestJury  
  Abbinata a ogni contest section, è un elenco di giurati che
  compongono la giuria. id giuria uuid, id section, id concorso
  (anche se basta tecnicamente solo section id), uuid del giurato. Questo è un dato che i circuiti
  non hanno ma i concorsi sì.  
