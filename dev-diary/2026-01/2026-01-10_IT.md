...
## FIAF Report 2 Foto ed Esiti

* nome concorso (in testata, a seguire)
* Patrocinio concorso
* Cognome Autore
* Nome autore
* Codice Fiscale
* Tessera FIAF
* Onorificenza FIAF (ma se ci son anche quelle FIAP va bene uguale)
* Sezione - vedi appunti precedenti,
* Tema - vedi appunti precedenti
* Titolo foto (con suffisso anno prima ammissione)
* Anno prima ammissione (dev'essere lo stesso indicato nel titolo)
* Esito Giuria (ammessa si/no) con valori 1 (is_admit) / 0
* Eventuale premio Assegnato

La spaccatura del codice sezione è relativa perché
gestendo la piattaforma solo concorsi digitali
la sezione sarà sempre **DIG**

1. creare la view (e cancellare il suo component)
2. creare la route
3. creare il controller
4. creare l'export che termina con il download della view

A differenza del precedente report questo è più "lineare"
e coinvolge:
...
```sql
SELECT pcp_contests.federation_list, 
       pcp_user_contacts.last_name,
       pcp_user_contacts.first_name,
       pcp_user_contacts_more.field_value, // codice fiscale
       pcp_user_contacts_more.field_value, // tessera
       pcp_user_contacts_more.field_value, // onorificenze
       'DIG' as section,
       pcp_contest_section.code,
       pcp_works.title_en,
       pcp_works.title_en, // per la parte finale dell'anno ammissione | anno del concorso quando manca
       pcp_contest_works.is_admit,
       pcp_contest_awards.award_name
...
```

Anche qui ci sarà bisogno di un traduttore perché
come nel precedente rapporto molti campi sono in diverse tabelle,
e perché 3 colonne arrivano dalla tabella user_contacts_more.
