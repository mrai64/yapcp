...
return: 0 (= false) non ho inserito, c'era già la chiave  
return: 1 (= true ) sì, ho inserito, la chiave non c'era
...
Sì, quelli con l'iniziale R prendono le liste per il didietro, e ci sta: REAR.

## Il set non ammette doppioni

Quindi non c'è ordine, non c'è primo ultimo FIFO LIFO, solo IN & OUT.  
`SADD`  
`SREM`  

`SISMEMBER` - c'è o manca  
`SMEMBERS chiave` - l'elenco dei membri  
`SCARD chiave` - La cardinalità, quanti sono.  
...
valori abbinati a una chiave e un secondo valore hash, che può anche essere
