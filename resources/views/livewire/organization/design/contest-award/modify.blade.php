...
new class () extends Component {
    ...
    // contest_id
    public ?string $contestAwardSectionId   = null; //   section_id     nullable
    public ?string $contestAwardSectionCode = null; //   section_code   nullable
    public string  $contestAwardAwardCode   = ''; //     award_code     sortable
    public string  $contestAwardAwardName   = ''; //     award_name
    public bool    $contestAwardIsAward     = false; //  is_award       bool
    //
    public function mount(ContestAward $contest_award)
    {
        // 1st: you can:update?
        $this->authorize('update', [ContestAward::class, $contest_award]);
        //
        $this->contestAward = $contest_award;
        $this->contest = $contest_award->contest;
        $this->organization = $this->contest->organization;
        //
        $this->contestAwardSectionId   = $this->contestAward->section_id; //   can be null
        $this->contestAwardSectionCode = $this->contestAward->section_code; // can be null
        //
    ...
    public function modifyContestAward()
    {
        ...
        // ..or create
        if ($this->contestAward->award_code <> $validated['contestAwardAwardCode']) {
        ...
        $contestAward = ContestAward::withTrashed()->updateOrCreate(
            [
                ...
                'section_id' => $this->contestAwardSectionId,
                'award_code' => $validated['contestAwardAwardCode'],
            ],
            [
                'award_name' => $validated['contestAwardAwardName'],
                'is_award'   => (bool) $validated['contestAwardIsAward'],
                'deleted_at' => null, // for soft-deleted record
            ]
        );
