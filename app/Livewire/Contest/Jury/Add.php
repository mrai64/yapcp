...
class Add extends Component
{
    ...
    public function mount(string $sid) // as indicate in route section_id
    {
        ...
        Log::info( __CLASS__.' '.__FUNCTION__ .':'. __LINE__ . ' '. json_encode($this->section) );
        
        $this->contest_id = $this->section['contest_id'];
        $this->contest = Contest::where('id', $this->contest_id)->get()[0];
        Log::info( __CLASS__.' '.__FUNCTION__ .':'. __LINE__ . ' '. $this->contest_id );
        ...
        Log::info( __CLASS__.' '.__FUNCTION__ . ' ' . __LINE__ . ' contest_section_list: ' . $this->contest_section_list);
        ...
        Log::info( __CLASS__.' '.__FUNCTION__ . ' ' . __LINE__ . ' contest_section_list: ' . json_encode($this->contest_section_list) );
    ...
    */
    public function add_contest_juror()
    {
        ...
        Log::info( __CLASS__.' '.__FUNCTION__ . ' ' . __LINE__. ' is_in: ' . $is_in_user_contact);
        // missing
        if ($is_in_user_contact === 0) {
            Log::info( __CLASS__.' '.__FUNCTION__ .':'. __LINE__ . ' try to insert ' );
            ...
            Log::info( __CLASS__.' '.__FUNCTION__ .':'. __LINE__ . ' user_contact:' . json_encode($user_contact) );

        } else {
            Log::info( __CLASS__.' '.__FUNCTION__ .':'. __LINE__ . ' found ' );
        ...
        // add to userRole
        Log::info( __CLASS__.' '.__FUNCTION__ . ' ' . __LINE__. ' user->id: '. $user->id);
