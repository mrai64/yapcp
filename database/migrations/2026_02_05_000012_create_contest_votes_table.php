...
return new class () extends Migration {
    ...
    public function up(): void
    {
        Schema::create('contest_votes', function (Blueprint $table) {
            $table->id()->comment('real pk: section_id + contest_work_id + juror_id');
            //
            ...
            $table->char('contest_work_id', 36)->charset('ascii')->collation('ascii_general_ci')
                ->index()->comment('fk: contest_works.work_id - NOT user_works.id');
            ...
            $table->dateTime('updated_at')->useCurrentOnUpdate()->useCurrent()
                ->index()->comment('date of vote');
            $table->dateTime('deleted_at')->nullable()->index();
            // idx
            $table->unique(['contest_id', 'section_id', 'contest_work_id', 'juror_user_id'], 'general_idx');
            $table->index(['section_id', 'juror_user_id', 'review_required', 'contest_work_id'], 'review_idx');
            $table->index(['contest_id', 'section_id', 'vote', 'contest_work_id', 'juror_user_id'], 'vote_idx');
            // fk
            $table->foreign(['contest_id'])->references(['id'])->on('contests')
                ->onUpdate('restrict')->onDelete('restrict');
            $table->foreign(['section_id'])->references(['id'])->on('contest_sections')
                ->onUpdate('restrict')->onDelete('restrict');
            $table->foreign(['contest_work_id'])->references(['id'])->on('contest_works')
                ->onUpdate('restrict')->onDelete('restrict');
            $table->foreign(['juror_user_id'])->references(['id'])->on('user_contacts')
                ->onUpdate('restrict')->onDelete('restrict');
            //
            $table->comment('The Jury vote board');
        });
    }
